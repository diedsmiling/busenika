{* $Id: step_four.tpl 10320 2010-08-03 11:47:28Z angel $ *}
...
<div class="step-container{if $edit}-active{/if}" id="step_four">
	<h2 class="step-title{if $edit}-active{/if}">
		...
		{if $edit && $cart|fn_allow_place_order}
			...
			<div class="darkish_loader" style="margin-top: 10px; margin-right: 35px; display: none;"><img src="/loading.gif"></div>
			
				{include file="buttons/place_order.tpl" but_onclick="$('#place_order').click(); $('#wrap_place_order').hide(); $('.button-big').hide(); $('.darkish_loader').show();" but_role="big"} 
			</span>
		{/if}
	</h2>
				...
				{if $cart|fn_allow_place_order}
					...
					{if $cart_agreements || $settings.General.agree_terms_conditions == "Y"}
						...
						<div class="buttons-container right clear-both">
								...
								<div class="darkish_loader" style="margin-top: 10px; margin-right: 35px; display: none;"><img src="/loading.gif"></div>
							{include file="buttons/place_order.tpl" but_onclick="$('#wrap_place_order').hide(); $('.button-big').hide(); $('.darkish_loader').show();" but_name="dispatch[checkout.place_order]" but_role="big" but_id="place_order"}
						</div>
