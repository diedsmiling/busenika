...
<div class="step-container{if $edit}-active{/if}" id="step_two">
	<h2 class="step-title{if $edit}-active{/if}">
		...
		<a class="title{if $complete && !$edit} cm-ajax{/if}" {if $complete && !$edit}href="{"checkout.checkout?edit_step=step_two&amp;from_step=`$edit_step`"|fn_url}" rev="checkout_steps"{/if}>{$lang.address}</a>
	</h2>
