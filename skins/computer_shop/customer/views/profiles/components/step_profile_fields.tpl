{* $Id: step_profile_fields.tpl 10269 2010-07-29 11:35:41Z angel $ *}
...
<div class="step-complete-wrapper clear">
	...
	{else}
		...
		{if $user_title || $user_firstname || $user_lastname}<p class="no-padding"><strong>{if $user_title}{$user_title} {/if}{$user_firstname}{if $user_lastname} {$user_lastname}{/if}</strong></p>{/if}
		...
		{if $user_zipcode}<p class="no-padding">{$user_zipcode}</p>{/if}
