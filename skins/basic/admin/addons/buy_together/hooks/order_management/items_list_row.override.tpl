{* $Id: items_list_row.override.tpl 9353 2010-05-04 06:10:09Z klerik $ *}
...
<tr {if $cp.product_options}class="no-border"{/if}>
	...
	<td>
		<a href="{"products.update?product_id=`$cp.product_id`"|fn_url}">{$cp.product|unescape}</a></td>
	<td class="no-padding">
	...
	{else}
		...
		<tr>
			...
			<span {if $cp.stored_price == "Y"}class="hidden"{/if} id="db_price_{$key}">{include file="common_templates/price.tpl" value=$cp.original_price}</span>
...
<tr>
	<td>&nbsp;</td>
	<td colspan="{if $cart.use_discount}5{else}4{/if}">
		<strong>{$lang.buy_together}</strong>
	</td>
</tr>
	...
	{if $_product.extra.parent.buy_together == $key}
		<tr class="no-border">
			...
			<td>
				...
				<tr>
					...
					<input type="checkbox" name="cart_products[{$k}][stored_price]" value="Y" {if $_product.stored_price == "Y"}checked="checked"{/if} onclick="$('#db_price_{$k},#manual_price_{$k}').toggle();" class="checkbox" />
					</td>
					<td class="data-block" valign="middle">
					<span {if $_product.stored_price == "Y"}class="hidden"{/if} id="db_price_{$k}">{include file="common_templates/price.tpl" value=$_product.original_price}</span>
					<span {if $_product.stored_price != "Y"}class="hidden"{/if} id="manual_price_{$k}">{$currencies.$primary_currency.symbol}&nbsp;<input type="text" class="input-text" size="5" name="cart_products[{$k}][price]" value="{$_product.base_price}" /></span>
					</td>
				</tr>
				</table>
			</td>
		...
		<tr>
			...
			<td colspan="{if $cart.use_discount}5{else}4{/if}">
				...
				<div id="warning_{$k}" class="float-left notification-title-e hidden">&nbsp;&nbsp;&nbsp;{$lang.nocombination}</div>
			</td>
		</tr>
		{/if}
	{/if}
