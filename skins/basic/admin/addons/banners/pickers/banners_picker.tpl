{* $Id: banners_picker.tpl 9804 2010-06-16 13:11:34Z lexa $ *}
		...
		<div class="buttons-container">
			...
			{if !$extra_var}
				...
				{capture name=$extra_buttons}
					{include file="buttons/save_cancel.tpl" but_type="button" but_onclick="jQuery.submit_picker('#iframe_`$data_id`', '#add_item')" but_text=$lang.add_banners breadcrumbs="" hide_second_button=true}
				{/capture}
			{else}
				{assign var="_but_text" value=$lang.add_banners}
				{assign var="_act" value="#add_item"}
			{/if}
			{include file="buttons/save_cancel.tpl" but_type="button" but_onclick="jQuery.submit_picker('#iframe_`$data_id`', '`$_act`');" but_text=$_but_text cancel_action="close" extra=$smarty.capture.$extra_buttons hide_second_button=true}
		</div>
	{/capture}
