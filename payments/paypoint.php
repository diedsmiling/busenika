...
//
// $Id: paypoint.php 9088 2010-03-15 10:40:51Z 2tl $
//
...
if (defined('PAYMENT_NOTIFICATION')) {
	if ($mode == 'process') {
		...
		if (!empty($processor_data["params"]["password"])) {
			$md5_hash = md5("trans_id=" . $_REQUEST['trans_id'] . "&amount=" . $order_info['total'] . "&callback=" . "$current_location/$index_script?dispatch=payment_notification.process&payment=paypoint&order_id=$_REQUEST[order_id]" . "&" . $processor_data["params"]["password"]);
		} else {
		...
		echo "<html><body onLoad=\"javascript: self.location='" . "$current_location/$index_script?dispatch=payment_notification.notify&payment=paypoint&order_id=$_REQUEST[order_id]" . "'\"></body></html>";
		exit;
	...
	$msg = str_replace('[processor]', 'PayPoint Server', $msg);

	//$processor_data['params']['merchant_id'] .= "-jredir";  // unsupported way to get worked redirection from the paypoint server

	$deferred = !empty($processor_data['params']['deferred']) ? '<input type="hidden" name="deferred" value="' . $processor_data['params']['deferred'] . '">' : '';
...
<html>
 <body onLoad="document.process.submit();">
  <form action="https://www.secpay.com/java-bin/ValCard" method="POST" name="process">
	...
	<input type="hidden" name="callback" value="{$current_location}/{$index_script}?dispatch=payment_notification.process&payment=paypoint&order_id={$order_id}">
