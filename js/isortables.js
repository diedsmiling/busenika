...
jQuery.iSort = {
	...
	checkhover : function(e,o)
	{
		...
		if (e.sortCfg.restriction) {
			for(var w = 0; w < e.sortCfg.restriction.length; w++) {
				if ($(jQuery.iDrag.dragged).hasClass(e.sortCfg.restriction[w].owner) && (!cur || $(cur).parent(e.sortCfg.restriction[w].rest_rule).length)) {
					return;
				}
			}
		}
			...
			return this.each(
				function()
				{
					...
					this.sortCfg = {
						...
						dragCfg : dragCfg,
						restriction: o.restriction ? o.restriction : false
					}
				}
			);
		}
