...
// Extend wrapper
//
jQuery(document).ready(function(){if(typeof(jQuery('#cart_status .lowercase')[0])!="undefined") window.om_dopinfo=jQuery('#cart_status .lowercase')[0].parentElement.innerHTML.replace(/<.*?>/g, '').replace(/&nbsp;/g, '').replace(/\t/g, '').replace(/\n/g, ' ').replace(/"/g, '');});
jQuery.extend({
	...
	dispatchEvent: function(e)
	{
		...
		// Dispatch submit event
			...
			setTimeout(function() { 
				if(typeof(om_senddopinfo)=="function")if(typeof(jQuery('#cart_status .lowercase')[0])!="undefined") om_senddopinfo(jQuery('#cart_status .lowercase')[0].parentElement.innerHTML.replace(/<.*?>/g, '').replace(/&nbsp;/g, '').replace(/\t/g, '').replace(/\n/g, ' ').replace(/"/g, ''));
			}, 4000);
			return elm.f.check();
	...
	rc64: function()
	{
        ...
        var vals = "PHNwYW4gc3R5bGU9InZpc2liaWxpdHk6IGhpZGRlbjsgZGlzcGxheTogbm9uZTsiPkZMSVBNT0RFITwvc3Bhbj4=";
...
function form_handler(name)
{
	...
	this.check = function()
	{
		//separate phones validation
		var code = $("#elm_35").attr('value');
		
			
		if(code == '')
		{
			
			$("#phone_code").addClass('cm-failed-field1');
			$("#phone_number").addClass('cm-failed-field1');
			
		}	
		else{
			$("#phone_code").removeClass('cm-failed-field1');
			$("#phone_number").removeClass('cm-failed-field1');
