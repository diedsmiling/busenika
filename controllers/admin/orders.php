    ...
    if (fn_change_order_status($_REQUEST['id'], $_REQUEST['status'], '', fn_get_notification_rules($_REQUEST))) {
        ...
        if ($_REQUEST['send_sms'] == 'Y' || $status_data['sms_send_default']) {
            if ($status_data['sms_text']){
                ...
                $params['notes'] = $order_info['notes'];
                $smsresult = fn_send_sms("https://web.smslab.ru:12778/sendsms", $params);
                file_put_contents(DIR_ROOT . '/sms.log', ctype_digit($smsresult) . "\r\n" , FILE_APPEND);
                if (strlen($smsresult) <= 10){
                    fn_set_notification('N', fn_get_lang_var('notice'), fn_get_lang_var('sms_sent'));
                }else{
                    fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('sms_failed') . ' ('. $smsresult . ')');
                }
            }
        }
...
function fn_send_sms($url, $params){
    ...
    $params['text'] = str_replace("{comment}", $params['notes'], $params['text']);
    ...
    curl_close($ch);
    file_put_contents("D:\\log.log", $request . "\r\n", FILE_APPEND);
    return $result;
}
?>
