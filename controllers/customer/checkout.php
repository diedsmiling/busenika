...
if (((true == fn_cart_is_empty($cart) && !isset($force_redirection)) || empty($payment_methods)) && !in_array($mode, array('clear', 'delete', 'cart', 'update', 'apply_coupon', 'shipping_estimation', 'update_shipping', 'complete'))) {
	if (empty($payment_methods)) {
		fn_set_notification('N', fn_get_lang_var('notice'),  fn_get_lang_var('cannot_proccess_checkout_without_payment_methods'), false, 'no_payment_notification');
	} else {
...
if ($mode == 'cart') {
	...
	if (!empty($cart_products)) {
		foreach($cart_products as $k => $v) {
			fn_gather_additional_product_data($cart_products[$k], true, true, true, false);
		}
	}
...
// Step 3: Shipping and payment methods
	...
	if (isset($cart['shipping_failed']) && !empty($cart['shipping_failed'])) {
		$completed_steps['step_four'] = false;
		fn_set_notification('N', fn_get_lang_var('notice'), fn_get_lang_var('text_no_shipping_methods'));
	}
