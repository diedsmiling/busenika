...
//
// $Id: func.php 10544 2010-08-30 13:22:19Z alexions $
//
...
{
	return str_replace('[admin_index]', Registry::get('config.admin_index'), fn_get_lang_var('clear_cache_info'));
}
			...
			foreach ($combinations as $c_id => $c_value) {
				...
				foreach ($c_value as $option_id => $variant_id) {
					if ($poptions[$option_id]['variants'][$variant_id]['modifier_type'] == 'A') {
						...
						$m_price += $products[$k]['price'] * $poptions[$option_id]['variants'][$variant_id]['modifier'] / 100;
						$m_weight += $products[$k]['weight'] * $poptions[$option_id]['variants'][$variant_id]['weight_modifier'] / 100;
					}
				}
				
				$product['combination_prices'][$c_id] = $product['price'] + $m_price;
