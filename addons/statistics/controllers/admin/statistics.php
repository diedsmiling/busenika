...
//
// $Id: statistics.php 10525 2010-08-26 09:42:29Z 2tl $
//
...
//
	...
	if (!empty($_REQUEST['section'])) {
		fn_add_breadcrumb(fn_get_lang_var($_REQUEST['section']), "statistics.reports?reports_group=$_REQUEST[section]&report=$report");
	...
	if ($reports_group == 'system') {
		...
		Registry::set('navigation.tabs', array (
			'operating_systems' => array (
				...
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'operating_systems')),
				'ajax' => true
			),
			'browsers' => array (
				'title' => fn_get_lang_var('browsers'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'browsers')),
				'ajax' => true
			),
			'resolutions' => array (
				'title' => fn_get_lang_var('resolutions'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'resolutions')),
				'ajax' => true
			),
		));
	...
	// Geo reports
		...
		Registry::set('navigation.tabs', array (
			'languages' => array (
				...
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'languages')),
				'ajax' => true
			),
			'ip_addresses' => array (
				'title' => fn_get_lang_var('ip_addresses'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'ip_addresses')),
				'ajax' => true
			),
			'countries' => array (
				'title' => fn_get_lang_var('countries'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'countries')),
				'ajax' => true
			),
		));
		...
		Registry::set('navigation.tabs', array (
			'by_domain' => array (
				...
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'by_domain')),
				'ajax' => true
			),
			'by_search_engine' => array (
				'title' => fn_get_lang_var('by_search_engine'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'by_search_engine')),
				'ajax' => true
			),
			'search_words' => array (
				'title' => fn_get_lang_var('search_words'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'search_words')),
				'ajax' => true
			),
			'came_from' => array (
				'title' => fn_get_lang_var('came_from'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'came_from')),
				'ajax' => true
			),
			'came_to' => array (
				'title' => fn_get_lang_var('came_to'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'came_to')),
				'ajax' => true
			),
			'all_referrers' => array (
				'title' => fn_get_lang_var('all'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'all_referrers')),
				'ajax' => true
			),
		));
		...
		Registry::set('navigation.tabs', array (
			'titles_by_visits' => array (
				...
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'titles_by_visits')),
				'ajax' => true
			),
			'entry_points' => array (
				'title' => fn_get_lang_var('entry_points'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'entry_points')),
				'ajax' => true
			),
			'exit_points' => array (
				'title' => fn_get_lang_var('exit_points'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'exit_points')),
				'ajax' => true
			),
			'pages_by_visits' => array (
				'title' => fn_get_lang_var('pages_by_visits'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'pages_by_visits')),
				'ajax' => true
			),
		));
		...
		Registry::set('navigation.tabs', array (
			'site_attendance' => array (
				...
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'site_attendance')),
				'ajax' => true
			),
			'visit_time' => array (
				'title' => fn_get_lang_var('visit_time'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'visit_time')),
				'ajax' => true
			),
			'repeat_new_visits' => array (
				'title' => fn_get_lang_var('repeat_new_visits'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'repeat_new_visits')),
				'ajax' => true
			),
			'page_load_speed' => array (
				'title' => fn_get_lang_var('page_load_speed'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'page_load_speed')),
				'ajax' => true
			),
			'total_pages_viewed_per_visitor' => array (
				'title' => fn_get_lang_var('total_pages_viewed_per_visitor'),
				'href' => "$index_script?" . fn_stat_build_query($_REQUEST, array('report' => 'total_pages_viewed_per_visitor')),
				'ajax' => true
			),
		));
		...
		$limit = fn_paginate($page, $total, $params['limit'], 'general_pagination_content');
		$data = array();
...
function fn_stat_get_visitors($params, $extra_join, $condition)
{
	...
	$data = db_get_hash_array("SELECT ?:stat_sessions.*, ?:stat_browsers.browser, ?:stat_browsers.version as browser_version, ?:stat_ips.*, phrase, ?:stat_languages.language, country FROM ?:stat_sessions $filter_condition WHERE $where GROUP BY ?:stat_sessions.sess_id ". (empty($sorting) ? '' : "ORDER BY $sorting") ." $limit", 'sess_id');
