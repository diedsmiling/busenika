	...
	EditArea.prototype.update_size= function(){
		...
		if( typeof editAreas != 'undefined' && editAreas[editArea.id] && editAreas[editArea.id]["displayed"]==true){
			if(editArea.fullscreen['isFull']){	
				...
				pd.getElementById("template_editor_content").style.width = pd.getElementsByTagName("html")[0].clientWidth + "px";
				pd.getElementById("template_editor_content").style.height = pd.getElementsByTagName("html")[0].clientHeight + "px";
			...
			height	= d.body.offsetHeight - editArea.get_all_toolbar_height() - 2;
